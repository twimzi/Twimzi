...
void updateThumbnail(File? image) {
state = state.copyWith(
thumbnail: image,
);
}

void updateGalleryImages(List<File> images) {
state = state.copyWith(
galleryImages: List<File>.from(images),
);
}

void addGalleryImage(File image) {
final images = List<File>.from(state.galleryImages)
..add(image);
